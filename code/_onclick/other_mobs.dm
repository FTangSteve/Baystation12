...
/mob/living/carbon/human/RangedAttack(var/atom/A)
	...
	if((istype(A, /turf/simulated/floor) || istype(A, /turf/unsimulated/floor) || istype(A, /obj/structure/lattice) || istype(A, /obj/structure/catwalk)) && isturf(loc) && above_shadow && !is_physically_disabled()) //Climbing through openspace
		var/turf/T = get_turf(A)
		var/turf/above = above_shadow.loc
		if(T.Adjacent(above_shadow) && above.CanZPass(src, UP)) //Certain structures will block passage from below, others not
			...
			above_shadow.visible_message("<span class='notice'>[above_shadow] starts climbing onto \the [A]!</span>")
			if(do_after(src, 50, A))
				...
				above_shadow.visible_message("<span class='notice'>[above_shadow] climbs onto \the [A]!</span>")
				src.Move(T)
			else
				...
				above_shadow.visible_message("<span class='warning'>[above_shadow] gives up on trying to climb onto \the [A]!</span>")
			return
